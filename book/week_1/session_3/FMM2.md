# Frequently made mistakes

## 1.1 Determine the normal force in bar $\rm{CD}$ and $\rm{AC}$

```{figure} ./answer_data/structure1.svg
:align: center
```

### No free-body diagrams

![alt text](FMM2_data/image_27.png)

### 'Incorrect' free-body diagrams

![alt text](./FMM2_data/image_21.png)

![alt text](./FMM2_data/image_26.png)

### Missing forces in free-body diagrams

![alt text](./FMM2_data/image_25.png)

![alt text](./FMM2_data/image_28.png)

### Incorrect lever arm

![alt text](./FMM2_data/image_23.png)

### Resultant / Equilibrium

![alt text](./FMM2_data/image_22.png)

### Missing equilibrium equations

![alt text](./FMM2_data/image_20.png)

![alt text](./FMM2_data/image_24.png)

## 1.2 Determine the displacement of node $\rm{C}$

### Trying to use forget-me-nots

![alt text](./FMM2_data/image_11.png)

### Incorrectly calculated resultant displacement

![alt text](./FMM2_data/image_2.png)

![alt text](./FMM2_data/image_29.png)

## 2.1 Determinate the support reaction at $\rm{D}$

```{figure} ./FMM_data/2.1.svg
:align: center
```

### FBD and supports

Similar to 1.1

### Incorrect FBD of part of structure

![alt text](./FMM2_data/image_38.png)

![alt text](./FMM2_data/image_39.png)

### Starting with V-line

![alt text](./FMM2_data/image_35.png)

![alt text](./FMM2_data/image_37.png)

## 2.2 Determinate the bending moment in $\rm{C}$ using virtual work

### No virtual work

![alt text](./FMM2_data/image_30.png)

![alt text](./FMM2_data/image_36.png)

![alt text](./FMM2_data/image_40.png)

## 2.3 Draw the bending moment diagram

### Bending moment diagram doesn't match loads

![alt text](./FMM2_data/image_31.png)


## 2.4 Draw the shear stress distribution just left of $\rm{C}$

### Wrong formula for $\tau$

![alt text](./FMM2_data/image_32.png)

## 2.5 Determine the displacement at $\rm{S}_2$

### Missing influence of internal forces

![alt text](./FMM2_data/image_33.png)

### Not using assumption of small displacements

![alt text](./FMM2_data/image_34.png)

## 2.6 Sketch the displaced structure without values

![alt text](./FMM2_data/image_19.png)